...
if method == 'GR-QAOA':
    if sim_method == 'noisy':
        output_csv = f'results/n{n_inst}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_2026_pow{power}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
    else:
        output_csv = f'results/n{n_inst}_{sim_method}_no_renorm_pen{penalty}_{method}_2026_pow{power}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
elif method == 'SGIR-QAOA-0k':
    if sim_method == 'noisy':
        output_csv = f'results/n{n_inst}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_{eig_meth}_2026_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
    else:
        output_csv = f'results/n{n_inst}_{sim_method}_no_renorm_pen{penalty}_{method}_{eig_meth}_2026_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
else:
    if sim_method == 'noisy':
        output_csv = f'results/n{n_inst}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_2026_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
    else:
        output_csv = f'results/n{n_inst}_{sim_method}_no_renorm_pen{penalty}_{method}_2026_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}.csv'
...
instances = sorted(
    [
        ...
        and int(match.group(1)) == n_inst
    ],
    key=lambda name: int(re.search(r"_(\d+)\.gpickle$", name).group(1))  # sort by instance number
)



for ppp in range(p_min,p_max,p_step):
    ...
    for instance in instances:
        ...
        if weighted_inst == 'weighted':
            if method == 'GR-QAOA':
                path = f'results/raw_results/n{n_inst}_n{n}_{p_e}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_pow{power}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}_p{p}.csv'
            if method == 'GR-QAOA_inv':
                path = f'results/raw_results/n{n_inst}_n{n}_{p_e}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_rate{tanh_rate}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}_p{p}.csv'   
            elif method == 'SGIR-QAOA-0k':
                path = f'results/raw_results/n{n_inst}_n{n}_{p_e}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_{eig_meth}_rate{tanh_rate}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}_p{p}.csv'   
            
            else:
                path = f'results/raw_results/n{n_inst}_n{n}_{p_e}_{sim_method}_pnoise{p_noise}_no_renorm_pen{penalty}_{method}_b{log_delta_beta_range}_g{log_delta_gamma_range}_d{disc}_p{p}.csv'
